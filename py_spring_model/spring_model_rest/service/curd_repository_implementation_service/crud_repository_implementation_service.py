...
from py_spring_model.spring_model_rest.service.curd_repository_implementation_service.method_query_builder import MetodQueryBuilder, Query
...
class CrudRepositoryImplementationService(Component):
    ...
    def _implemenmt_query(self, repository_type: Type[CrudRepository]) -> Any:
        ...
        for method in methods:
            ...
            def create_wrapper(service: "CrudRepositoryImplementationService", query: Query) -> Any:
                def wrapper(*args, **kwargs) -> Any:
                    ...
                    for key, value in kwargs.items():
                        if value is None:
                            raise ValueError(f"Invalid field name: {key}, received None value.")
                    
                    logger.info(f"Executing query condition: {query.conditions}, is_one_result={query.is_one_result}: {kwargs}")
                    # Execute the query
            ...
            setattr(repository_type, method, wrapped_method.__get__(repository_type, repository_type.__class__))

            
    def find_by(self, model_type: Type[SQLModel], conditions: list[str], is_one_result: bool, **kwargs) -> Any:
        """
        ...
        for cond in conditions:
            if cond == "_and_":
                ...
                filters.append(getattr(model_type, cond) == kwargs.get(current_query_field))
